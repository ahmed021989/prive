...
$count=$bd->num_rows($SQL);
	...
	if (empty($errors)){
 		...
 		if ($edit->save()){
			$sql=$bd->requete("update employer set archive=0 where id_employe=".$_GET['id_employe']."");
		$msg_positif .= '<p style= "font-size: 20px; ">  Il a été ajouter la nouvelle contrat  de ' . (htmlspecialchars_decode($edit->nom_employe)) .'    </p><br />';
...
<input class="form-control " id="type_etablissement"  name="type_etablissement" readonly  required style= "text-align:left;color :#666;font-size:16px" value="<?php $etab=Etablissement::trouve_par_id($employer->type_etablissement); echo $etab->type_etab;?>"  />													   
	...
	<button type="button"  onclick="liste_etab();" class='btn btn-success'>cliquez ici pour choisir</button>
            ...
            <div class="pull-right">
				  ...
				  ?>
                        ...
                        <center><h3><p> </p></h3></center>
						  <table id="table3" class="table datatable table-striped" style="Maw-width:none !important;direction:rtl;text-align:left" >
						...
						<tr style="font-weight:bold" id ="<?php echo htmlspecialchars_decode($employer->id_employe); ?>"> 
											...
											<?php if($employer->type_etablissement==0){ ?>
											 <a style="color:#green;font-size:14px" id="<?php echo $employer->id_employe;?>"  class=" btn btn-warning 
" data-toggle="tooltip"  href="edit_employer_pere.php?id_employe=<?php echo $employer->id_employe; ?>" target="_blanck">Modifier avant de choisir</a> 
											<?php } else { ?>
					 <button style="color:#green;font-size:20px" id="<?php echo $employer->id_employe;?>"  class=" btn btn-success fa fa-building-o
" data-toggle="tooltip"  onClick="ici(<?php echo $employer->id_employe; ?>);">Choisi</button> &nbsp &nbsp
	<?php }  ?>
											...
											<td><?php
												...
												echo stripcslashes(htmlspecialchars_decode($etablissement->type_etab)); else echo "<span style='color:red'>VIDE</span>" ?></td>
											<td><?php
