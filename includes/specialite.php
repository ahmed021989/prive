...
class Specialite {
	...
	protected static $nom_table="specialite";    
	protected static $champs = array('id_specialite', 'nom_specialite','diplome','id_groupe_specialite');
	public $id_specialite;
	public $nom_specialite;
	public $diplome;

public $id_groupe_specialite;
...
{ 
  ...
  return round($resultat); // Arrondi la valeur
	...
	public static function trouve_tous() {
  	...
  	public static function trouve_specialite($specialite) {
			$sp="";
		if($specialite[0]=="tous"){$sp.="id_specialite like '%'  ";}
		
		if($specialite[0]!="tous"){$sp.='id_specialite = "'.$specialite[0].'"';}	
	if(sizeof($specialite)>1){
		for($i=1;$i<sizeof($specialite);$i++){
			
			$sp.=' or id_specialite = "'.$specialite[$i].'"';
		}
	}
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where ".$sp."");
  }
  
  
    public static function trouve_par_id($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_specialite={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
     public static function trouve_par_groupe($id) {
    return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_groupe_specialite={$id} ");
		//return !empty($result_array) ? array_shift($result_array) : false;
  }
  
   public static function trouve_par_groupe0($id) {
    $result_array =self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_groupe_specialite={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_nom_specialite($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE nom_specialite  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  
   public static function trouve_par_nom($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE nom  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
...
$sql .= " WHERE id_specialite =". $bd->escape_value($this->id_specialite) ;
$bd->requete($sql);
