...
class Diplome {
	...
	protected static $nom_table="diplome";    
	protected static $champs = array('id_diplome', 'nom_diplome');
	public $id_diplome;
	public $nom_diplome;
...
{ 
  ...
  return round($resultat); // Arrondi la valeur
    ...
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_diplome={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_nom_diplome($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE nom_diplome  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
...
$sql .= " WHERE id_diplome =". $bd->escape_value($this->id_diplome) ;
$bd->requete($sql);
