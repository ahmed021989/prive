...
class Communes {
	...
	protected static $champs = array('id_com', 'code_postal', 'nom_com', 'wilaya_id','pop_com');
	public $id_com;
	public $code_postal;
	public $nom_com;
public $wilaya_id;
...
{ 
  ...
  public static function trouve_tous_wilaya($id_w) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table."  where wilaya_id=".$id_w."");
  }
  
  
    public static function trouve_par_id($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_com={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_code_postal($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code_postal  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  
   public static function trouve_par_nom($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE nom_com  ='{$id}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
