...
class Affiliation {
	...
	protected static $nom_table="affiliation";    
	protected static $champs = array('id_employe', 'nom_pere','nom_mere','photo');
	public $id_employe;
	public $nom_pere;
	public $nom_mere;
	public $photo;
...
{ 
  ...
  return round($resultat); // Arrondi la valeur
	...
	public  function  existe($id_employe){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE id_employe = ".$this->id_employe." ";
   ...
   public static function trouve_par_nom_pere($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE nom_pere  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
