...
class Wilayas {
	...
	protected static $champs = array('id_w', 'code', 'nom','superficie','pop_wil');
	public $id_w;
	public $code;
	public $nom;
	public $superficie;
...
{ 
  ...
  //founction trouve wilaya
  	public static function trouve_wilaya($wilaya) {
			$w="";
		if($wilaya[0]=="tous"){$w.="id_w like '%'  ";}
		
		if($wilaya[0]!="tous"){$w.='id_w = "'.$wilaya[0].'"';}	
	if(sizeof($wilaya)>1){
		...
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where ".$w."");
  }
  
  
  
  
  public static function trouve_par_Nom($id) {
