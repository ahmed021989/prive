...
class Actualite {
	...
	protected static $nom_table="actualite";
	protected static $champs = array('id_act', 'nom_act', 'contenu','publier');
	public $id_act;
	public $nom_act;
	public $contenu;
	public $publier;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE nom_act = '".$this->nom_act."' ";
  ...
  public static function trouve_par_id($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_act ={$id} limit 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_act)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_act =". $bd->escape_value($this->id_act) ;
$bd->requete($sql);
