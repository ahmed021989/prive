...
class Transfere {
	...
	protected static $nom_table="transfere";
	protected static $champs = array('id_trans', 'id_employe', 'date_trans','commune_installation','adrs','type_etablissement','identite_jurdique','date_agriment','numero_agriment','date_creation','date_instal');
	public $id_trans;
	public $id_employe;
	public $date_trans;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE id_employe =-1 ";
	$sql .= "AND date_trans = '".$this->date_trans."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}
  ...
  public static function trouve_par_id($id) {
    $result_array =self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_employe ={$id}");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_trans)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_trans =". $bd->escape_value($this->id_trans) ;
$bd->requete($sql);
