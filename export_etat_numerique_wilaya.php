...
if(!$session->is_logged_in()) {
 ...
 if  ($user->type == 'administrateur' or $user->type == 'Admin_dsp' or $user->type == 'DGSS-RH')   {   
         ...
         list($specialite1, $commune1,$recherche) = explode("|",$_GET['var']); 
  

$specialite=  explode(",",$specialite1);

$commune=  explode(",",$commune1);

$excel = "";
$excel .= "".utf8_decode("Spécialité")."\t Nbre\t Homme\t Femme\t <25 Ans\t 25-34\t 35-44\t 45-54\t 55-64\t >65 Ans\n";

									$total=0;
									$specialites=Specialite::trouve_specialite($specialite);	
								foreach($specialites as $specialite){
									$wil=Wilayas::trouve_par_Nom($user->wilaya);
									if($employer = Employe::trouve_par_specialite_dsp($specialite->id_specialite,$wil->id_w,$commune)){
									//$total+= $employer->count_specialite($specialite->id_specialite,$wilaya);
$result=$employer->count_specialite_dsp_interval_age($specialite->id_specialite,$wil->id_w,$commune,$recherche);	
