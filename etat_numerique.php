...
?>
 ...
 <!-- START BREADCRUMB -->
                <ul class="breadcrumb">
                    ...
                    <li class="active">Tableau de bord > Etat numérique</li>
                </ul>
                <!-- END BREADCRUMB -->                       
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default">
								...
								function filtre($taille,$user,$wilaya,$specialite,$recherche){
...
if(sizeof($wilaya)>1){
		for($i=1;$i<sizeof($wilaya);$i++){
			...
			$w.=' et '.htmlspecialchars(trim($wilay->nom)).'"';
								...
								<center><label class="alert alert-success" id="liste" style="font-size:12px;background:#14de29"><span class='pull-left'>Etat numérique pour  </span><?php echo " wilaya :<span style='color:blue'>".$w."</span>&nbsp;&npsp".$r;; ?>
							<?php if($user->type!="DGSS-RH"){?>	
						<a  href='export_etat_numerique.php?var=<?php  foreach($specialite as $ss){ echo $ss.",";} echo "|" ; foreach($wilaya as $ww) { echo $ww.",";}  echo "|".$recherche ; ?>' class='btn btn-danger pull-right' ><img width="20" src="img/icons/xls.png"> Export excel</a>
							<?php }?>
						</label>
								</center>
						<div  class="scrollable0" id="scrol" >                                   
								 <table id="table00" class="table   table-striped" style="background:#0f9a1a2e;color:#000;font-weight:bold">
			...
			if($recherche=="s"){$r=" and (type_employe=-1 or type_employe=0)";}
								foreach($specialites as $specialite){
									...
									if($employer = Employe::trouve_par_specialite($specialite->id_specialite,$wilaya)){
										//$result = $employer->count_specialite_interval_age($specialite->id_specialite,$wilaya); 
									$sql=$bd->requete("select count(*) as moins25 from employer WHERE specialite=".$specialite->id_specialite." and (".$w.")  and date_nais_employe>'".$date_moins_25."-01-01' ".$r." and archive=0 ");
									$sql1=$bd->requete("select count(*) as 25_34 from employer WHERE specialite=".$specialite->id_specialite." and (".$w.")  and ( date_nais_employe<='".$date_entre_25."-12-31' and date_nais_employe>='".$date_entre_34."-01-01' ) ".$r." and archive=0 ");
									$sql2=$bd->requete("select count(*) as 35_44 from employer WHERE specialite=".$specialite->id_specialite." and (".$w.")  and ( date_nais_employe<='".$date_entre_35."-12-31' and date_nais_employe>='".$date_entre_44."-01-01' ) ".$r." and archive=0 ");
									$sql3=$bd->requete("select count(*) as 45_54 from employer WHERE specialite=".$specialite->id_specialite." and (".$w.")  and ( date_nais_employe<='".$date_entre_45."-12-31' and date_nais_employe>='".$date_entre_54."-01-01') ".$r." and archive=0 ");
									$sql4=$bd->requete("select count(*) as 55_64 from employer WHERE specialite=".$specialite->id_specialite." and (".$w.")  and ( date_nais_employe<='".$date_entre_55."-12-31' and date_nais_employe>='".$date_entre_64."-01-01') ".$r." and archive=0 ");
									$sql5=$bd->requete("select count(*) as plus65 from employer WHERE specialite=".$specialite->id_specialite." and (".$w.")  and  date_nais_employe<='".$date_plus_65."-12-31' ".$r." and archive=0 ");
									$sql6 = $bd->requete("select count(*) as h from employer WHERE specialite=".$specialite->id_specialite." and (".$w.") and sexe_employe='homme' ".$r." and archive=0"); 
									$sql7 = $bd->requete("select count(*) as f from employer WHERE specialite=".$specialite->id_specialite." and (".$w.") and sexe_employe='femme' ".$r." and archive=0"); 
									$sql8 =$bd->requete("select count(*) as tous from employer WHERE specialite=".$specialite->id_specialite." and (".$w.") ".$r." and archive=0"); 
									...
									<td><?php echo $specialite->nom_specialite;   ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql8))  echo  number_format($row['tous'], 0, ',', ' '); ; 
										//$total+=  $result[8];  ?></td>
										
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql6)) echo number_format($row['h'], 0, ',', ' '); ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql7)) echo number_format($row['f'], 0, ',', ' ');  ?></td>
										
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql)) echo  number_format($row['moins25'], 0, ',', ' '); ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql1)) echo  number_format($row['25_34'], 0, ',', ' '); ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql2)) echo  number_format($row['35_44'], 0, ',', ' '); ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql3)) echo  number_format($row['45_54'], 0, ',', ' '); ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql4)) echo  number_format($row['55_64'], 0, ',', ' '); ?></td>
										<td style="text-align:right"><?php while($row=mysqli_fetch_array($sql5)) echo  number_format($row['plus65'], 0, ',', ' '); ?></td>
...
if(isset($_POST['submit'])){
	...
	filtre(0,$user,$wilaya,$specialite,$_POST['recherche']);
		}else {}					
 } else {
	if( !empty($_POST['specialite']) &&  !empty($_POST['wilaya'])){
	filtre(100,$user,"tous","tous","tous",$_POST['recherche']);
			}
   ...
   <!-- PAGE CONTENT WRAPPER -->
                <div class="page-content-wrap">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default">
								...
								<select  class="select" data-live-search="true" id="commune" name="commune[]" multiple="multiple" title="Selectioner communes" >
					...
					<div class="col-md-2 pull-right">								
									<button type="submit" name="submit2" style="border-radius:10px;background:#20820d" class="btn btn-success btn-lg fa fa-search pull-right"  > afficher</button>
							</div>	
								...
								<?php 
								//CREATION DE LA FOUNCTION FILTRE
								function filtre2($taille,$user,$wilaya,$commune,$specialite,$recherche){
...
//******************************											
	...
	//*********************************	
								$com="";
		if($commune[0]=="tous"){$com.="toutes les communes";}
		
		if($commune[0]!="tous"){
			$comm=Communes::trouve_par_code_postal($commune[0]);
			$com.=htmlspecialchars(trim($comm->nom_com));
			}	
	if(sizeof($commune)>1){
		for($i=1;$i<sizeof($commune);$i++){
			
			$comm=Communes::trouve_par_code_postal($commune[$i]);
		...
		$r="";
			if($recherche=="s"){$r="recherche structures ";}else { $r=="recherche tous" ;}
						...
						<div  class="scrollable0" id="scrol" >    
<center><label class="alert alert-success" id="liste" style="font-size:12px;background:#0D9E1C"><span class='pull-left'>Etat numérique pour  </span><?php echo " communes :<span style='color:#fff'>".$com."</span>&nbsp;&nbsp; |".$r ; ?>
								<a  href='export_etat_numerique_wilaya.php?var=<?php  foreach($specialite as $ss){ echo $ss.",";} echo "|" ; for($i=0;$i<sizeof($commune)-1;$i++) { echo $commune[$i].",";} echo $commune[sizeof($commune)-1];  echo "|".$recherche ; ?>' class='btn btn-danger pull-right' ><img width="20" src="img/icons/xls.png"> Export excel</a>
		...
		$r="";
			if($recherche=="s"){$r=" and (type_employe=-1 or type_employe=0)";}
								foreach($specialites as $specialite){
									...
									$sql=$bd->requete("select count(*) as moins25 from employer WHERE specialite=".$specialite->id_specialite." and (".$com.")  and date_nais_employe>'".$date_moins_25."-01-01' ".$r." and archive=0 ");
									$sql1=$bd->requete("select count(*) as 25_34 from employer WHERE specialite=".$specialite->id_specialite." and (".$com.")  and ( date_nais_employe<='".$date_entre_25."-12-31' ".$r." and date_nais_employe>='".$date_entre_34."-01-01' ) and archive=0 ");
									$sql2=$bd->requete("select count(*) as 35_44 from employer WHERE specialite=".$specialite->id_specialite." and (".$com.")  and ( date_nais_employe<='".$date_entre_35."-12-31' ".$r." and date_nais_employe>='".$date_entre_44."-01-01' ) and archive=0 ");
									$sql3=$bd->requete("select count(*) as 45_54 from employer WHERE specialite=".$specialite->id_specialite." and (".$com.")  and ( date_nais_employe<='".$date_entre_45."-12-31' ".$r." and date_nais_employe>='".$date_entre_54."-01-01') and archive=0 ");
									$sql4=$bd->requete("select count(*) as 55_64 from employer WHERE specialite=".$specialite->id_specialite." and (".$com.")  and ( date_nais_employe<='".$date_entre_55."-12-31' ".$r." and date_nais_employe>='".$date_entre_64."-01-01') and archive=0 ");
									$sql5=$bd->requete("select count(*) as plus65 from employer WHERE specialite=".$specialite->id_specialite." and (".$com.")  and  date_nais_employe<='".$date_plus_65."-12-31' ".$r." and archive=0 ");
									$sql6=$bd->requete("select count(*) as h from employer WHERE specialite=".$specialite->id_specialite." and (".$com.") and sexe_employe='homme' ".$r." and archive=0"); 
									$sql7=$bd->requete("select count(*) as f from employer WHERE specialite=".$specialite->id_specialite." and (".$com.") and sexe_employe='femme' ".$r." and archive=0"); 
									$sql8=$bd->requete("select count(*) as tous from employer WHERE specialite=".$specialite->id_specialite." and (".$com.") ".$r."  and archive=0"); 
...
$wilaya=Wilayas::trouve_par_Nom($user->wilaya);
	...
	filtre2(0,$user,$wilaya->id_w,$_POST['commune'],$specialite,$_POST['recherche']);
...
for(var i = 1; i < elements.length; ++i){
 ...
 $('#specialite').selectpicker('refresh');
		...
		$("#commune").on('change',function(){
			 var specialites = [];
        $.each($("#commune option:selected"), function(){            
            specialites.push($(this).val());
        });

		if(specialites[0]=='tous'){

	var elements = document.getElementById("commune").options;
for(var i = 1; i < elements.length; ++i){
